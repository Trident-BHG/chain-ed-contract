...
import {IVRFV2PlusWrapper} from "../vrf/IVRFV2PlusWrapper.sol";
import {VRFV2PlusWrapperConsumerBase} from "../vrf/VRFV2PlusWrapperConsumerBase.sol";
    ...
    IVRFV2PlusWrapper
{
    ...
    error IncorrectExtraArgsLength(
        uint16 expectedMinimumLength,
        uint16 actualLength
    );
    ...
    function _calculateRequestPriceNative(
        ...
        uint256 coordinatorCostWithPremiumAndFlatFeeWei = ((coordinatorCostWei *
            (s_coordinatorNativePremiumPercentage + 100)) / 100) +
    ...
    function _calculateRequestPrice(
        ...
        return
            (1e18 *
                (wrapperCostWei + coordinatorCostWithPremiumAndFlatFeeWei)) /
    ...
    function onTokenTransfer(
        ...
        if (isFeedStale) {
            emit FallbackWeiPerUnitLinkUsed(
                requestId,
                s_fallbackWeiPerUnitLink
            );
        }
    ...
    function checkPaymentMode(
        ...
        if (extraArgs.length < EXPECTED_MIN_LENGTH) {
            revert IncorrectExtraArgsLength(
                EXPECTED_MIN_LENGTH,
                uint16(extraArgs.length)
            );
        }
        ...
        returns (uint256 requestId)
    {
        ...
        uint256 price = _calculateRequestPriceNative(
            _callbackGasLimit,
            _numWords,
            tx.gasprice
        );
    ...
    function _getFeedData()
        private
        view
        returns (int256 weiPerUnitLink, bool isFeedStale)
    {
        uint32 stalenessSeconds = s_stalenessSeconds;
        uint256 timestamp;
        (, weiPerUnitLink, , timestamp, ) = i_link_native_feed
            .latestRoundData();
        // solhint-disable-next-line not-rely-on-time
        isFeedStale =
            stalenessSeconds > 0 &&
            stalenessSeconds < block.timestamp - timestamp;
        if (isFeedStale) {
